...
const Checkout = () => {
  ...
  const [selectedAddress, setSelectedAddre] = useState<Address | null>(null);
  ...
  const fetchAddress = async () => {
    ...
    if (addressList.length > 0) {
      ...
      setSelectedAddre(def as Address);
    }
  ...
  const handlePlaceOrder = async () => {
    if (!selectedAddress) {
      Toast.show({
        type: "error",
        text1: "Error",
        text2: "Please add Shippping Address ",
      });
      return;
    }
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface" edges={["top"]}>
        ...
        >
          ...
          <View className="ml-3 flex-1">
            ...
            <Text className="text-secondary text-xs mt-1 ">
              Pay when you recived the order
            </Text>
          </View>
          {paymentMethod === "cash" && (
            <Ionicons
      ...
      {/* Order Summary */}
      <View className="p-4 b-white text-lg border-t border-gray-100">
        <Text className="text-lg font-bold text-primary mb-4 ">
          Order Summary
        </Text>
        {/* Sub Total */}
